...
const Simulation: React.FC<{ caseData: ICase.Self }> = ({ caseData }) => {
  ...
  return (
    <View className="relative flex flex-1">
      <View
        className={cn(
          "absolute left-0 top-0 -z-10 h-screen w-screen -translate-x-4 -translate-y-4",
          environment
        )}
      />
      <View className={"flex-1 items-center justify-center gap-4"}>
        <View className="mb-4 items-center space-y-2 text-center">
          <Text className="font-display text-2xl font-bold">{caseData.name}</Text>
        ...
        <SimulationActions medicalCase={caseData} messages={[]} />
      </View>
      <SimulationVitals />
    </View>
  );
